...
const CustomInput = ({
  ...
  const [passwordIcon,setPasswordIcon] = useState('google');
  ...
  const handleIconPress = () => {
    ...
    setPasswordIcon((prevIcon) => (prevIcon === 'google' ? 'rocket' : 'rocket'));
    if (onIconPress) {
      onIconPress();
    }
    if(emailverified){
    ...
    <View style={[styles.container, { marginBottom: marginBottom || 15}]}>
      ...
      />
        ...
        <TouchableOpacity onPress={handleIconPress}>
          <Icon
           name={icon}
           size={24} color={fontColor || '#000'} style={[{padding:5}]}/>
        </TouchableOpacity> 
      )}
      ...
      {isEmailValid && (
        <TouchableOpacity onPress={handleIconPress}>
          <Icon name={"google"} size={24} color={fontColor || '#000'} style={[{padding:5}]}/>
        </TouchableOpacity>
      )}
